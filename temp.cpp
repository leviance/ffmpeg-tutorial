...
int main(int argc, char * argv[])
{
    ...
    // open audio device
    if(SDL_OpenAudio(&wanted_specs, nullptr) < 0) {
...
void packet_queue_init(PacketQueue * q)
{
    ...
    // Returns the initialized and unlocked mutex or nullptr on failure
    ...
    // Returns a new condition variable or nullptr on failure
...
int packet_queue_put(PacketQueue * q, AVPacket * pkt)
{
    ...
    auto * avPacketList = (AVPacketList*)av_malloc(sizeof(AVPacketList));
...
static int packet_queue_get(PacketQueue * q, AVPacket * pkt, int block)
{
    ...
    for (;;)
    {
        ...
        // if the first packet is not nullptr, the queue is not empty
        if (avPacketList)
        {
            ...
            if (!q->first_pkt)
            {
                // first_pkt = last_pkt = nullptr = empty queue
                q->last_pkt = nullptr;
            }
...
)
{
    ...
    printf("out channel layout: %d", out_channels);
